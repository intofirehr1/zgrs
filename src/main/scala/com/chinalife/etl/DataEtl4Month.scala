...
object DataEtl4Month {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 设置运行参数： cpu, mem
    val conf = new SparkConf().setAppName("gs_data_etl_month").setMaster(args(0))
    ...
    val kmCols = spark.sqlContext.table(Commons.whole_table_customs).columns.mkString("", ",", "").toUpperCase
    spark.sqlContext.sql("INSERT OVERWRITE TABLE " + Commons.whole_table_customs +
  ...
  def parseData(spark: SparkSession, featureRangDF: DataFrame): DataFrame = {
    val cols = spark.table(Commons.whole_table_sources_tmp).columns.mkString("", ",", "").toUpperCase()
    val tmpDF = spark.sqlContext.sql("SELECT " + cols + " FROM " + Commons.whole_table_sources_tmp)
  ...
  def generKM(spark: SparkSession, msgDF: DataFrame, k: Int, maxK:Int, runs: Int,
    ...
    val tmpschema = "TRANS_CODE,TRANS_BAT_NO,ACC_CUST_KM"
    ...
    val predictResult = msgDF.join(kmDF, Seq("TRANS_CODE", "TRANS_BAT_NO"), "left_outer")
